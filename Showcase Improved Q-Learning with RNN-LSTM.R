...
strat <- c("strat.a","strat.b","strat.c","strat.d","strat.e","strat.f","strat.g","strat.h","strat.i") #"strat.a" to "strat.i" with get.antistrat or any other strategy (vector) without "self" implies self play.
antistrat <- "none" #or "none" if none of the strategies of above or several strategies are given.
file.name <- paste0("opt.run.",paste0(strat,collapse="."),".",Sys.Date(),".RNN") #File, where the results are saved
...
block.no <- 1000 #Number of Blocks to Play. More should be always better, but time increases somewhat linear (given the memory is sufficiently full), while we have strong diminishing return in the final performance depending on the complexity of the strategy.
...
## batch.size.train is the RNN internal size of how big a neural network batch should be (see https://stats.stackexchange.com/questions/153531/what-is-batch-size-in-neural-network). More means more stable, but slower/more epochs needed.
##rnn.epochs is the number of how often the complete training data should be propagated through the network. More means more overfitting but better accuracy in describing the training data. Very relevant for speed.
##give.up.precision controls a method used in the thesis of Martin Kies: After training epochs times it is checked whether the new LOSS is better than the one in the block before. If not the training is repeated up to give.up.precision times. The actual number of epochs such may vary between rnn.epochs and rnn.epochs*give.up.precision
func.approx.params <- list(nodes.layer.1=126, nodes.layer.2=64, batch.size.train=600, rnn.epochs=5, give.up.precision=10) 
...
generate.best.strat <- function(strat, antistrat="none", game.setting, func.approx.params, memory.initialization, block.no, eval.no, T.max, file.name){
  ...
  game.object <- Get.Game.Object.PD(encoding.state="TimeSeries.minimal",game.setting=game.setting,strats=strat, eval.strategy = "Model.strat.RNN.TimeSeries.minimal")
  ...
  if(antistrat!="none"){
    strat.tourn = nlist(Model.strat.RNN.TimeSeries.minimal,get(strat), get(antistrat))
    names(strat.tourn)[2] <- strat
    names(strat.tourn)[3] <- antistrat
  } else {
    strat.tourn = c(Model.strat.RNN.TimeSeries.minimal,lapply(strat,FUN=function(x){
      if(x!="self"){
        return(get(x))
      } else {
        return(NULL)
  ...
  if(length(strat)==1){
    r.limit <- get.matches.vs.matrix(tourn$dt)["Model.strat.RNN.TimeSeries.minimal",strat]
  } else {
    srfm <- strat.rank.from.matches(tourn$dt)
    r.limit <- srfm[srfm$strat=="Model.strat.RNN.TimeSeries.minimal",mean]
  }
